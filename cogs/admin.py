...
class Admin(commands.Cog):
    ...
    async def switchmap(self, ctx, map_id: str, game_mode: str, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Switched to map `{map_id}` with game mode `{game_mode}` on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to switch map: `{e}`"))
    ...
    async def maplist(self, ctx, server_name: str = None):
        ...
        try:
            ...
            embed = info_embed(f"**Current map rotation on `{server_name}`:**\n```\n{response}\n```")
            await ctx.send(embed=embed)
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to get maplist: `{e}`"))
    ...
    async def addmaprotation(self, ctx, map_id: str, game_mode: str, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Added map `{map_id}` with game mode `{game_mode}` to the rotation on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to add map to rotation: `{e}`"))
    ...
    async def removemaprotation(self, ctx, map_id: str, game_mode: str, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Removed map `{map_id}` with game mode `{game_mode}` from the rotation on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to remove map from rotation: `{e}`"))
    ...
    async def setmaxplayers(self, ctx, amount: int, server_name: str = None):
        ...
        if amount < 1 or amount > 24:
            await ctx.send(embed=warn_embed("Player count must be between 1 and 24."))
            return
        try:
            await send_rcon(server_name, "SetMaxPlayers", str(amount))
            await ctx.send(embed=success_embed(f"Set maximum players to `{amount}` on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set max players: `{e}`"))
    ...
    async def updateservername(self, ctx, *, name: str):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Updated server name to `{name}` on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to update server name: `{e}`"))
    ...
    async def rotatemap(self, ctx, server_name: str = None):
        ...
        if not has_server_permission(ctx, server_name, required="modroles"):
            ...
            return
        try:
            response = await send_rcon(server_name, "RotateMap")
            await ctx.send(embed=info_embed(f"**Rotate map result on `{server_name}`:**\n```\n{response}\n```"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to rotate map: `{e}`"))
    ...
    async def moderatorlist(self, ctx, server_name: str = None):
        ...
        if not has_server_permission(ctx, server_name, required="modroles"):
            ...
            return
        try:
            response = await send_rcon(server_name, "ModeratorList")
            await ctx.send(embed=info_embed(f"**Moderator list on `{server_name}`:**\n```\n{response}\n```"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to get moderator list: `{e}`"))
    ...
    async def shutdownserver(self, ctx, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=warn_embed(f"Server `{server_name}` is shutting down..."))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to shutdown server: `{e}`"))
    ...
    async def pausematch(self, ctx, seconds: int, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Paused match for `{seconds}` seconds on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to pause match: `{e}`"))
    ...
    async def resetsnd(self, ctx, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"SND match has been reset on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to reset SND match: `{e}`"))
    ...
    async def setpin(self, ctx, pin_number: str, server_name: str = None):
        ...
        try:
            ...
            if pin_number.lower() == "none":
                await ctx.send(embed=success_embed(f"Server join pin has been removed on `{server_name}`"))
            else:
                await ctx.send(embed=success_embed(f"Server join pin has been set to `{pin_number}` on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set pin: `{e}`"))
    ...
    async def settimelimit(self, ctx, seconds: int, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Match time limit set to `{seconds}` seconds on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set time limit: `{e}`"))
    ...
    async def shownametags(self, ctx, enabled: bool, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Friendly nametags have been **{state}** on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set nametags: `{e}`"))
    ...
    async def enablecompmode(self, ctx, enabled: bool, server_name: str = None):
        ...
        try:
            ...
            state = "enabled" if enabled else "disabled"
            await ctx.send(embed=success_embed(f"Competitive mode has been **{state}** on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set competitive mode: `{e}`"))
    ...
    async def enablewhitelist(self, ctx, enabled: bool, server_name: str = None):
        ...
        try:
            ...
            state = "enabled" if enabled else "disabled"
            await ctx.send(embed=success_embed(f"Whitelist has been **{state}** on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set whitelist: `{e}`"))
    ...
    async def setlimitedammotype(self, ctx, ammo_type: int, server_name: str = None):
        ...
        if ammo_type < 0 or ammo_type > 5:
            await ctx.send(embed=warn_embed("Ammo type must be between 0 and 5."))
            return
        try:
            await send_rcon(server_name, "SetLimitedAmmoType", str(ammo_type))
            await ctx.send(embed=success_embed(f"Set ammo limitation type to `{ammo_type}` on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to set ammo type: `{e}`"))
    ...
    async def removemod(self, ctx, unique_id: str, server_name: str = None):
        ...
        try:
            ...
            await ctx.send(embed=success_embed(f"Removed moderator `{unique_id}` on `{server_name}`"))
        except Exception as e:
            await ctx.send(embed=error_embed(f"Failed to remove moderator: `{e}`"))
